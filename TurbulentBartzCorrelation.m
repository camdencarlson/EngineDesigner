...
M = 0.207; % mach number
m = 0.6; % temperature dependence of viscosity
...
To = 1 + (gamma-1)/2*M^2; % stagnation temp degK
...
Cp_throat = Cp_kJ_kg_throat * 1000;
pressure_throat = pressure_throat * 6894.757293;

%% get yvals from Contour.m
avals = yvals .^2 .*pi;
...
% contourf(contur)
y = 0:1/length(xvals):1;
figure()
[X, Y] = meshgrid(xvals,y(1:end-1));
Z = ones(length(xvals),length(y(1:end-1)));
...
view(0,90)
c = colorbar;
c.Label.String = 'W/m2/degC';
title('Heat Transfer Coefficient Over Engine Length')
xlabel('x [inches]')
